...
@router.post("/applications/{application_id}/contact-unlock")
async def request_contact_unlock(
    ...
    unlock = await contact_unlock_service.create_unlock_request(
        ...
        requester_id=current_user.get("principal_id", ""),
    )
    await db.commit()
    return {"unlock_id": unlock.id, "status": unlock.status}


@router.post("/applications/{application_id}/contact-unlock/authorize")
async def authorize_contact_unlock(
    ...
    unlock = await contact_unlock_service.candidate_authorize(
        db=db,
        unlock_id=unlock.id,
        candidate_id=current_user.get("principal_id", ""),
        reason=reason,
    )
    await db.commit()
    return {"unlock_id": unlock.id, "status": unlock.status}
...
@router.post("/applications/{application_id}/contact-unlock/unlock")
async def do_unlock_contact(
    ...
    unlock = await contact_unlock_service.employer_unlock(
        db=db,
        unlock_id=unlock.id,
        employer_id=current_user.get("principal_id", ""),
        reason=reason,
    )
    await db.commit()
    return {"unlock_id": unlock.id, "status": unlock.status}
...
@router.get("/applications/{application_id}/contact-unlock")
async def get_contact_unlock_status(
    ...
    unlock = await contact_unlock_service.get_by_application(db, application_id)
    if not unlock:
        return {"status": None, "message": "No unlock request"}
    return {
        "unlock_id": unlock.id,
